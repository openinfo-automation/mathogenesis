...
startBtn.onclick = function() {
    fetch("/start_agents", {method: "POST"});
    startBtn.disabled = true;
    startBtn.textContent = "AGENTS RUNNING";
};
...
eventSource.onmessage = function(e) {
    ...
    div.className = data.milestone ? "milestone" : "discovery";
    container.prepend(div);
    document.getElementById("agentsCount").textContent = data.agents;
    document.getElementById("theoremsCount").textContent = data.proven;
    document.getElementById("conjecturesCount").textContent = data.novel;
};
</script>
</body>
</html>
...
def generate_conjecture():
    ...
    # Slight random mutation
    if random.random() < 0.4:
        base = base.replace("a+b", "(a+1)+b")
    return base
...
def agent_loop():
    ...
    while running:
        ...
        if proved and is_novel:
            ...
            discoveries.append({"text": f"🎓 NEW THEOREM PROVED: {conjecture} via {method}",
                                "milestone": True})
            proven_theorems_count += 1
        elif is_novel and not proved:
            discoveries.append({"text": f"💡 Novel Conjecture Generated: {conjecture}",
                                "milestone": False})
        ...
        while True:
            while last_index < len(discoveries):
                ...
                payload = {
                    ...
                    "proven": proven_theorems_count,
                    "novel": novel_conjectures_count
                }
